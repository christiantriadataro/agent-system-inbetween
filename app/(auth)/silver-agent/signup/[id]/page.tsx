...
import PlatRegistration from "@/public/Silver Agent Registration.svg"
...
const Register = () => {
    ...
    const handleSubmit = async (event: FormEvent) => {
        try {
            ...
            const response = await postSilverUser({...newGoldUser, parent: params.id})
            console.log("New Silver Agent Created", response.data);
            console.log("Silver Signup Success");
            router.push("/silver-agent/login");
        } catch (error: any ) {
            console.log(error)
        }
    }
    ...
    return (
        <div
            className={` background-primary-silver w-full border border-black h-screen flex flex-col justify-evenly gap-10 pb-20`}>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-[30px] items-center mx-4 md:mx-6 lg:mx-10">
                ...
                <CustomButton variant="silver" text="Register"/>
            </form>
        </div>
    )
}
