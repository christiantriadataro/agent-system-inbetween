...
import PlatRegistration from "@/public/Player Registration.svg"
...
const Register = () => {
    ...
    const handleSubmit = async (event: FormEvent) => {
        try {
            ...
            const response = await postPlayer({...newGoldUser, parent: params.id})
            console.log("New Player Agent Created", response.data);
            console.log("Player Signup Success");
    ...
    return (
        <div
            className={` background-primary-player w-full border border-black h-screen flex flex-col justify-evenly gap-10 pb-20`}>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-[30px] items-center mx-4 md:mx-6 lg:mx-10">
                ...
                <CustomButton variant="player" text="Register"/>
            </form>
        </div>
    )
}
