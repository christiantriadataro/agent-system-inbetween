...
const Login = () => {
    ...
    const handleSubmit = async event => {
        try {
            ...
            const response = await PlatLogin(username, password);
            console.log("Login success", response.data);
            router.push("/platinum-agent/dashboard");
        } catch (error: any) {
            console.log(error);
        }
    }
  ...
  return (
    <div className="background-primary-plat w-full border border-black h-screen flex flex-col justify-end pb-20 ">
      <form onSubmit={handleSubmit} className="flex flex-col gap-[26px] items-center mx-4">
              ...
              <ButtomPrimary text="Login" variant="plat"  />
      </form>
   </div>
  )
}
